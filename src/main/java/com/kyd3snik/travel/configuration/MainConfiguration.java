...
class MainConfiguration {
    ...
    public CommandLineRunner test(HotelRepository hotelRepository, HotelRoomRepository hotelRoomRepository,
        ...
        return args -> {
            ...
            Hotel hotel6 = hotelRepository.save(new Hotel(0, "Sanremo", city2, "ул. Черноморская, 13Г", (byte) 4, List.of(hotelRoom10)));
            Resort resort1 = resortRepository.save(new Resort(0, "Поездка в Москву", "Непродолжительное путешествие в столицу России с экскурсией", city4, city1, List.of(tag6, tag7), hotel1, 3, DateUtil.getDate(2020, 10, 20), DateUtil.getDate(2020, 10, 23), 8300, (byte) 1));
            Resort resort2 = resortRepository.save(new Resort(0, "Поездка в столицу Марокко", "Увлекательное путешествие", city1, city5, List.of(tag6, tag7, tag2, tag5), hotel4, 7, DateUtil.getDate(2020, 6, 5), DateUtil.getDate(2020, 6, 12), 52000, (byte) 2));
            Resort resort3 = resortRepository.save(new Resort(0, "Отпуск в Турции", "Великолепный отдых на берегу Эгейского моря", city1, city7, List.of(tag2, tag5), hotel5, 5, DateUtil.getDate(2020, 7, 9), DateUtil.getDate(2020, 7, 14), 45000, (byte) 2));
            Resort resort4 = resortRepository.save(new Resort(0, "Отпуск в Сочи", "Поездка в Сочи", city1, city2, List.of(tag1, tag2, tag7), hotel6, 7, DateUtil.getDate(2020, 12, 5), DateUtil.getDate(2020, 12, 12), 42000, (byte) 2));
